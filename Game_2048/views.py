...
def message_board(request):
    ...
    context['hs'] = {}
    context['ranklist'] = [3, 4, 5, 6, 7, 8, 9, 2]
...
def playing(request):  #问题: 游戏可以通过返回键退回操作，所以可能用js写游戏好些？
    ...
    is_login = request.session.get('is_login')
    #if this_game[name] is None:
        #print(id(this_game[name]), this_game[name]) #TEST
    ...
    if request.is_ajax(): #new experiment
        ...
        if this_game[name].state == 1:
            this_game[name].move(dir_dict[direction])
        if not this_game[name].board.is_continue():
            this_game[name].over()
        print('after ajax:', this_game[name].to_json_with_last_move(direction)) #TEST
        response = HttpResponse(this_game[name].to_json_with_last_move(direction), content_type='application/json')
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            raise Http404('Invalid size.')
        this_game[name] = Game(name=name, size=size)
        form = DirectionForm()
    else:
        raise Http404('Invalid visit.')
    state = this_game[name].state
    context = {
        'form':form,
        'name':name,
        'size':size,
