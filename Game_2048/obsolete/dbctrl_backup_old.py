...
class MyDBCTRL:
    ...
    def create_HighestScore(self):
        ...
        create_HighestScore_str = \
            ...
            "rcid INT, " + \
            "CONSTRAINT PK_LoginRecord PRIMARY KEY (loginid), " + \
            "CONSTRAINT FK_LoginRecord_rcid FOREIGN KEY (rcid) REFERENCES PlayRecord(rcid), " + \
            "CONSTRAINT FK_LoginRecord_uid FOREIGN KEY (uid) REFERENCES User(uid), " + \
            ");"
        cursor.execute(create_LoginRecord_str)
    ...
    def check_login(self, name, pwd):
        # 在User中根据name, password查找该玩家的的记录，用于检查登录。
        # 返回是否存在该记录
        conn, cursor = self.aux_connect_sql_server_2008()
        find_player_str = \
            "SELECT uid FROM " + \
    ...
    def check_login(self, name, email):
        # 在User中根据name, email任一信息查找该玩家的的记录，用于检查注册。
        # 返回是否不存在该记录
        conn, cursor = self.aux_connect_sql_server_2008()
        find_player_str = \
            "SELECT uid FROM " + \
            "User WHERE name=? OR email=?;"
