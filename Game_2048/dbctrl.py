...
class MyDBCTRL:
    ...
    def create_PlayRecord(self):
        ...
        # rank: TINYINT NOT NULL (棋盘大小，2到9)
        ...
        create_PlayRecord_str = \
            ...
            "CHECK (score>=0), CHECK (rank<=9 AND rank >= 2)" + \
            ");"
    ...
    def create_HighestScore(self):
        ...
        # rank: TINYINT NOT NULL CHECK (rank>=2 AND rank<=9) (棋盘大小，2到9)
        ...
        create_HighestScore_str = \
            ...
            "rank TINYINT NOT NULL CHECK (rank>=2 AND rank<=9), " + \
            "uid INT NOT NULL, " + \
            "rcid INT, " + \
            "CONSTRAINT PK_HIGH_Record PRIMARY KEY (hsid), " + \
            "CONSTRAINT FK_HIGH_Record_rcid FOREIGN KEY (rcid) REFERENCES PlayRecord(rcid), " + \
            "CONSTRAINT FK_HIGH_Record_uid FOREIGN KEY (uid) REFERENCES UserInfo(uid) ON DELETE CASCADE, " + \
            ");"
        ...
        create_PlayRecord_trigger_str = \
            ...
            ")" + \
            "  BEGIN " + \
            "  UPDATE HighestScore SET rcid=(SELECT rcid FROM inserted)" + \
            "  WHERE hsid=@hsid;" + \
            "  END " + \
            "END"
        cursor.execute(create_PlayRecord_trigger_str)
        create_newHighestScore_trigger_str = \
            "CREATE TRIGGER NEW_HIGHESTSCORE ON UserInfo " + \
            "FOR INSERT AS " + \
            "BEGIN " + \
            "DECLARE @uid INT;" + \
            "SELECT @uid=uid FROM inserted;" + \
            "INSERT INTO HighestScore (rank, uid, rcid) VALUES (3, @uid, NULL);" + \
            "INSERT INTO HighestScore (rank, uid, rcid) VALUES (4, @uid, NULL);" + \
            "END"
        cursor.execute(create_newHighestScore_trigger_str)
        conn.commit()
        if self.aux_check_table_existence(cursor, 'HighestScore'):
            result = 'Success'
        else:
    ...
    def show_play_record(self, name):
        # 在PlayRecord中查找所有该玩家的游戏记录，rank, score, playtime，
        # 返回最近10条。
        conn, cursor = self.aux_connect_sql_server_2008()
        find_play_record_str = \
            "SELECT rank, score, playtime FROM " + \
            "PlayRecord INNER JOIN UserInfo ON PlayRecord.uid=UserInfo.uid " + \
    ...
    def show_login_record(self, name):
        # 在LoginRecord中查找所有该玩家的登录记录，logintime，
        # 返回最近10条。
        conn, cursor = self.aux_connect_sql_server_2008()
    ...
    def show_personal_highscore(self, name):
        # 在PlayRecord和HighestScore中根据name查找rank, score, playtime，按rank升序排列
        conn, cursor = self.aux_connect_sql_server_2008()
        find_highscore_str = \
            "SELECT HighestScore.rank, score, playtime FROM (PlayRecord " + \
            "INNER JOIN HighestScore ON HighestScore.rcid=PlayRecord.rcid ) " + \
            "INNER JOIN UserInfo ON HighestScore.uid=UserInfo.uid " + \
            "WHERE name=? ORDER BY HighestScore.rank ASC; "
        cursor.execute(find_highscore_str, name)
        return cursor.fetchall()

    def show_rank(self, name):
        # 在PlayRecord和HighestScore中根据name查找rank，按rank升序排列
        conn, cursor = self.aux_connect_sql_server_2008()
        find_highscore_str = \
            "SELECT rank FROM HighestScore " + \
            "INNER JOIN UserInfo ON HighestScore.uid=UserInfo.uid " + \
            "WHERE name=? ORDER BY rank ASC; "
        cursor.execute(find_highscore_str, name)
        return cursor.fetchall()
